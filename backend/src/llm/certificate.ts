...
import { anthropic, CERTIFICATE_TIMEOUT_MS, MAX_TOKENS, MODEL } from "./client.js";
...
export async function generateCertificate(session: Session): Promise<Certificate> {
  ...
  const response = await anthropic.messages.parse(
    ...
    {
      // 7턴 전체 채점 + 총평 생성이라 다른 호출보다 구조적으로 오래 걸림.
      // 클라이언트 기본 상한(30초) 대신 이 호출만 더 넉넉하게 잡는다.
      timeout: CERTIFICATE_TIMEOUT_MS,
    },
  );
